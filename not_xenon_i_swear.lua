...
--// CRS V3 Library ;o
function CRS.Utils.MakeUtilController(Settings)
    local Utils = {
        Tasks = {};
        Tweens = {};
        Services = {};
        ...
        Strings = {};
        Tables = {};
        Settings = Settings or {ConfigName = "coolrobloxscript/crsConfig.json"};
        LogFile = nil;
    }
    ...
    return setmetatable(Utils, CRS.Utils)
end

function CRS.Utils:Beautify(String)
    local Str = String
    for i = 3, #Str do
...
function CRS.Utils:Log(Text)
    if self.LogFile == nil then
        local num = 0
        repeat num = num + 1 until isfile("crsv1/Logs/crs1_ScriptLog_" .. num .. ".txt") == false
        self.LogFile = "crsv1/Logs/crs1_ScriptLog_" .. num .. ".txt"
        writefile(self.LogFile, "")
    end
    appendfile(self.LogFile, Text .. "\n")
end

function CRS.Utils:GetLogPath()
    return self.LogFile
end

function CRS.Utils:MakeFolder()
	if isfolder("crsv1") == false then 
        makefolder("crsv1")
    end
    if isfolder("crsv1/Logs") == false then 
        makefolder("crsv1/Logs")
    end
end

function CRS.Utils:ConvertConfig(Config)
...
function CRS.Utils:WriteData(Data)
	self:MakeFolder()
...
function CRS.Utils:GetInt(Value)
...
function CRS.Utils:GetString(Value)
...
function CRS.Utils:GetState(Value)
...
function CRS.Utils:GetTable(Value)
...
function CRS.Utils:SetInt(Value, NewValue)
    if self.Ints[Value] then
        self.Ints[Value].Value = NewValue
    end
end

function CRS.Utils:SetString(Value, NewValue)
    if self.Strings[Value] then
        self.Strings[Value].Value = NewValue
    end
end

function CRS.Utils:SetState(Value, NewValue)
    if self.States[Value] then
        self.States[Value].Value = NewValue
    end
end

function CRS.Utils:SetTable(Value, NewValue)
    if self.Tables[Value] then
        self.Tables[Value].Value = NewValue
    end
end

function CRS.Utils:ChangeTable(Value, TableIndex, NewValue)
    if self.Tables[Value] then
        self.Tables[Value].Value[TableIndex] = NewValue
    end
end

function CRS.Utils:InsertTable(Value, InsertedValue)
    if self.Tables[Value] then
        table.insert(self.Tables[Value].Value, InsertedValue)
    end
end

function CRS.Utils:RemoveTable(Value, RemovedValue)
    if self.Tables[Value] then
        table.remove(self.Tables[Value].Value, table.find(self.Tables[Value].Value, RemovedValue))
    end
end

function CRS.Utils:FindTable(Value, TableIndex)
    return table.find(self.Tables[Value].Value, TableIndex)
end

function CRS.Utils:SaveConfig()
...
function CRS.Utils:GetService(Service)
    return self.Services[Service]
end

function CRS.Utils:GetPlayer()
    return self.Services.Players.LocalPlayer
end

function CRS.Utils:GetCharacter()
    return self:GetPlayer().Character or self:GetPlayer().CharacterAdded:Wait()
end

function CRS.Utils:GetHumanoid()
...
function CRS.Utils:HasProperty(Part, Property)
...
function CRS.Utils:Compare(A, B)
...
function CRS.Utils:Identify(Item)
...
function CRS.Utils:WaitTillBackpackLoaded()
...
function CRS.Utils:CountItem(Item)
...
function CRS.Utils:Freeze(State)
    local Character = self:GetCharacter()
...
function CRS.Utils:Teleport(CF, Offset)
    local Character = self:GetCharacter()
    local FinalCF;
...
function CRS.Utils:GetDistance(Part)
    local Character = self:GetCharacter()
...
function CRS.Utils:AddToQueue(Item)
...
function CRS.Utils:Click(Button, Manual)
...
function CRS.Utils:LearnSkills(Skills)
...
function CRS.Utils:CheckStand()
    return self:GetPlayer().PlayerStats.Stand.Value
end

function CRS.Utils:Collect(Item)
    local Character = self:GetCharacter()
    ...
    self:Log("CRS:C_Clip: Added task.")
    ...
    self:Log("CRS:C_Clip: Removed Task.")
end

function CRS.Utils:FarmItems(List)
    for Prompt, Item in pairs(self:GetTable("Queue")) do
        ...
        if table.find(List, ItemName) and not self:IsMax(ItemName) then
            xpcall(function()
                    ...
                    warn("[CRS Error : Collect]:", err)
                    warn("[CRS Traceback]:", debug.traceback())
                    self:DisconnectTask("C_Clip")
            end)
        end
    end
...
function CRS.Utils:AddTask(TaskName, Task)
...
function CRS.Utils:IsTaskRunning(TaskName)
    if self.Tasks[TaskName] then
        return self.Tasks[TaskName].Connected
    else
        warn("[CRS Warning]: Task does not exist!")
        return
    end
end

function CRS.Utils:DisconnectTask(TaskName)
...
function CRS.Utils:SearchPlayer(Name)
...
function CRS.Utils:SendWebhook(webhook, msg, title)
...
function CRS.Utils:Censor(string)
...
function CRS.Utils:GetQuest(NPC)
...
function CRS.Utils:UseMove(Move)
    Move = Move
...
function CRS.Utils:ReplaceReset()
    local NewEvent = Instance.new("BindableEvent")
    NewEvent.Event:Connect(function()
        local Char = self:GetCharacter()

        if Char and Char:FindFirstChild("RemoteEvent") then
            Char.RemoteEvent:FireServer("Reset", {Anchored = false}, "CRS_ON_TOP");
        end
    end)
    self:GetService("StarterGui"):SetCore("ResetButtonCallback", NewEvent)
end

function CRS.Utils:Outline(Chars)
...
function CRS.Utils:LabelChar(Chars)
...
local Util = CRS.Utils.MakeUtilController()
local CRSConfig = Util:ReadData():LoadData()

Util:Log("CRS Debug - Passed Functions")
--// Preload Api ----
getgenv().XenApi = Util
Util:Log("CRS Debug - Preloaded API")
--[[ Load Captcha Bypass
...
--Util:Log("CRS Debug - Item Captcha Bypassed")
...
Util:Log("CRS Debug - Passed WS/Jump Hook")
...
OldNamecallTP = hookmetamethod(game, '__namecall', newcclosure(function(self, ...)
    ...
    if (Method == "FireServer" or Method == "InvokeServer" or Method == "InvokeClient") and Arguments[1] == "Reset" and Arguments[3] ~= "CRS_ON_TOP" then
        return wait(9e9) 
    end
...
Util:Log("CRS Debug - Passed TP Hook")
...
Util:Log("CRS Debug - Enabled Anti AFK")
...
Util:Log("CRS Debug - Enabled Auto-Rejoin on Kick")
--// Reset fix
...
Util:Log("CRS Debug - Replaced Reset")

--// Logs
Util:Log(("-"):rep(150) .. "\nDo not send this log file to anyone apart from CRS owners or developers in case of an error/bug in order to keep your account safe!\n[CRS Log]: Loaded Hooks/Misc functions in: " .. tick()-HookStart .. "s")
Util:Log("[CRS Log]: Username: " .. Util:GetPlayer().Name .. " - UserId: " .. Util:GetPlayer().UserId .. " - Log Path: " .. Util:GetLogPath())
Util:Log("[CRS Log]: User Config: " .. Util:GetService("HttpService"):JSONEncode(Util:ReadData().Data))

Util:Log("CRS Debug - Ran Execution Logfile")
--// Skip Loading Screen

if not Util:IsSBR() then
    Util:Log("CRS Debug - Loading HUD")
    ...
    Util:Log("CRS Debug - Skipped Loading Screen")
    Util:Log("[CRS Log]: Skipped Loading Screen")
    ...
    Util:Log("CRS Debug - Passed Loading Screen Checks")
...
Util:Log("CRS Debug - Passed SBR Check")
...
Util:Log("CRS Debug - Passed Item Hook")
...
if not Util:IsSBR() and game.PlaceId ~= 5290908008 then
    ...
    for i,v in pairs(workspace.Item_Spawns.Items:GetChildren()) do
        Util:AddToQueue(v)
        Util:Log("[CRS Queue]: Added " .. v.Name .. " to the item queue")
    end
...
Util:Log("CRS Debug - Added Items to Queue")
-------------------------------------------------------------------------------------------------------------------------------------------------------
...
Util:Log("CRS Debug - Passed Values")
--// Mod Check (998)
...
Util:Log("CRS Debug - Added Mod-Check")
...
Util:Log("CRS Debug - Bypassed Game Mechs")
...
Util:Log("CRS Debug - Created all Tabs")

--// Credits (1001 for ctrl + f)
local Credits_Developers = Credits:Section("Developers")
local Credits_Info = Credits:Section("Info")

Credits_Developers:Label("Main Developer: azwees")
Credits_Developers:Label("Co-Developer: gemini")
Credits_Developers:Label("UI Designer: cloudiAI")
Credits_Developers:Label("Extra Features: cool _vez0 and some shit")

Credits_Info:Label("Version: v0.0.0.1a")
Credits_Info:Label("Exploit: " .. (identifyexecutor and identifyexecutor() or "Unsupported exploit"))
Credits_Info:Button("Copy Xenon Discrod (thanks for source)", function()
    ...
    setclipboard("i dont have a discord server")
...
Util:Log("[CRS Task]: Loaded Credits Tab")

Util:Log("CRS Debug - Finished Credits Tab")
...
Util:Log("CRS Debug - Passed Player Tab Load")
...
Util:Log("CRS Debug - Passed Item Section")
...
Util:Log("CRS Debug - Passed Stand Farm Section")
...
Util:Log("CRS Debug - Parsed Quests")
...
Util:Log("CRS Debug - Passed Auto Choose Quests")
        ...
        if EnemyName then
            for i,v in pairs(workspace.Living:GetChildren()) do
                    ...
                    if v.Name == EnemyName then
                        ...
                        if F then
                            warn("[CRS] Kill function experienced an error; but was handled by CRS -", F)
                        end
                    end
            end
        else
...
Util:Log("CRS Debug - Passed Quest Section")
...
for i,v in pairs(workspace.Living:GetChildren()) do
    if v:FindFirstChild("Spawn") and Util:FindTable("AllNPCs", v.Name) == nil then
        ...
        Util:Log("CRS:3040: Added NPC to table (" .. v.Name .. ")")
    elseif v:FindFirstChild("Spawn") and Util:FindTable("AllNPCs", v.Name) ~= nil then
        --Util:Log("CRS:3042: (Above ID 1023) - Attempt to add NPC that already exists in table")
    end
end
...
workspace.Living.ChildAdded:Connect(function(child)
    if child:FindFirstChild("Spawn") and Util:FindTable("AllNPCs", child.Name) == nil then
        ...
        TP_C:UpdateAsset(Util:GetTable("AllNPCs"))
        Util:Log("CRS:3051: Added NPC to table (" .. child.Name .. ")")
    elseif child:FindFirstChild("Spawn") and Util:FindTable("AllNPCs", child.Name) ~= nil then
        --Util:Log("CRS:3053: (Above ID 1023) - Attempt to add NPC that already exists in table")
    end
end)
Util:Log("CRS Debug - Added NPCs to Table")
...
Util:Log("CRS Debug - Passed Quest Farm Section")
...
if not Util:IsSBR() then
    ...
    task.spawn(function()
        while task.wait(1) do
            ...
            if not S then
                warn("[CRS] Stand info had an error, but it was handled -", F)
            end
        end
    end)
end -- dont mind this
Util:Log("CRS Debug - Passed Stand Info Section")
...
Util:Log("CRS Debug - Created Teleports Sections")
...
Util:Log("CRS Debug - Added Players to Table")
...
Util:Log("CRS Debug - Added Teleports from Table")
...
local Misc_Dash = Player:Section("Dash")
...
local Misc_Anonimous = Misc:Section("Anon")

local WaypointStatus = Misc_Waypoints:Label("Waypoint: None")

Util:Log("CRS Debug - Created Misc Sections")
--[[
Misc_Webhooks:Toggle("Disable CRS Logs", Util:GetState("Disable CRS Logs"), function(State)
    Util:SetState("Disable CRS Logs", State)
    Util:SaveConfig();
end, "Disables from logging to CRS server")
...
Util:Log("CRS Debug - Passed Misc Trolling Asset Update")
...
Misc_Other:Toggle("Hide own name", false, function(state)
    ...
    if state == true then 
        Plr.PlayerGui.HUD.Playerlist[Plr.Name].PlayerName.Text = "CRS ON TOP!!!"
    else
...
Misc_Other:Toggle("Hide all player names", false, function(state)
    ...
    if state then
        for i,v in pairs(game.Players:GetPlayers()) do
            Plr.PlayerGui.HUD.Playerlist[v.Name].PlayerName.Text = "CRS ON TOP!!!"
            if v.Character then
...
Util:Log("CRS Debug - Passed Keybind Section")
if Util:IsSBR() then
...
Util:Log("CRS Debug - Passing Pressed Play Check")
if not Util:IsSBR() then
    Util:GetCharacter():WaitForChild("RemoteEvent"):FireServer("PressedPlay")  
end
Util:Log("CRS Debug - Passed Press Play Check")
...
Util:Log("CRS Debug - Added Anims to Table")
...
Util:Log("CRS Debug - Attempted to fix chat")

game:GetService("CoreGui").CRSV3Lib.TabHolder.Status.Label.Text = "Status: Loaded!"
